...
class HermesModelsTests(unittest.TestCase):
    def test_provider_choices_use_hermes_authenticated_provider_inventory(self):
        inventory = [
            {"slug": "openrouter", "name": "OpenRouter", "models": ["anthropic/claude-sonnet-4", "openai/gpt-5.4-mini"], "total_models": 2},
        ...
        self.assertEqual(
            choices,
            [
                ProviderChoice("openrouter", "OpenRouter — 2 models", ("anthropic/claude-sonnet-4", "openai/gpt-5.4-mini")),
    ...
    def test_provider_choices_can_use_selected_runtime_python_inventory(self):
        ...
        with (
            patch("hermes_stack_bootstrap.hermes_models._provider_rows_from_runtime", return_value=inventory) as runtime_rows,
    ...
    def test_provider_choices_fall_back_to_empty_list_when_hermes_modules_are_unavailable(self):
        with patch("hermes_stack_bootstrap.hermes_models.list_hermes_authenticated_providers", side_effect=ImportError("no hermes")):
    ...
    def test_model_choices_for_provider_prefers_selected_runtime_python_catalog(self):
        with (
            patch("hermes_stack_bootstrap.hermes_models._provider_model_ids_from_runtime", return_value=("runtime",)) as runtime_models,
